...
public class MysqlFakerCreator {
    ...
    private void executeMainLogic() throws Exception {
        ...
        fileContent.append("\t\t").append(String.format("DBTools.url(\"jdbc:mysql://localhost:3306/%s\")", dbName)).append("\n");
        fileContent.append("\t\t\t\t").append(String.format(".username(\"%s\")", username)).append("\n");
        fileContent.append("\t\t\t\t").append(String.format(".password(\"%s\")", password)).append("\n");
        fileContent.append("\t\t\t\t").append(".driverClassName(\"com.mysql.jdbc.Driver\")").append("\n");
    ...
    private static String createFakerTable(String tableName, List<FieldInfo> fieldInfoList) {
        ...
        for (FieldInfo fieldInfo : fieldInfoList) {
            sb.append(String.format("\t\t\t\t.param(\"%s\", %s)\n",
                    fieldInfo.getField(), fieldToInferType(fieldInfo)));
        }
    ...
    private static String fieldToInferType(FieldInfo fieldInfo) {
        // 字段名
        String fieldName = fieldInfo.getField().toUpperCase();
        // 删除括号后的字段类型
        String fieldType = removeBrackets(fieldInfo.getType().toUpperCase());
        // 是否是主键（值为PRI表示主键）
        String isPrimaryKey = fieldInfo.getKey();

        // 1.根据是否主键设置（字段名中有ID，并且字段类型是字符串类型，并且是主键）
        if (fieldName.contains("ID") && isStringType(fieldType) && "PRI".equals(isPrimaryKey)) {
            return "DataType.ID";
        }
